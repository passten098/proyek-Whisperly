<?php

namespace App\Http\Controllers;

use App\Modules\pengguna\Models\pengguna;
use App\Modules\talents\Models\TalentSchedule;
use App\Modules\talents\Models\talents;
use Illuminate\Http\RedirectResponse;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\Storage;
use Illuminate\View\View;

class WhisperlyTalentController extends Controller
{
    private const SLOTS = [
    ['08:00', '09:00'],
    ['09:00', '10:00'],
    ['10:00', '11:00'],
    ['11:00', '12:00'],
    ['13:00', '14:00'],
    ['14:00', '15:00'],
    ['15:00', '16:00'],
    ['16:00', '17:00'],
    ['17:00', '18:00'],
    ['18:00', '19:00'],
    ['19:00', '20:00'],
    ['20:00', '21:00'],
    ['21:00', '22:00'],
];

    public function index(): View
    {
        $talents = pengguna::query()
            ->where('role', 'talent')
            ->orderBy('username')
            ->get()
            ->map(fn (pengguna $user) => $this->profileFor($user));

        return view('whisperly.talents.index', compact('talents'));
    }

    public function show(string $username): View
    {
        $user = pengguna::query()
            ->where('role', 'talent')
            ->where('username', $username)
            ->firstOrFail();

        return view('whisperly.talents.show', [
            'talent' => $this->profileFor($user),
        ]);
    }

    public function own(Request $request): View
    {
        return view('talent', [
            'talent' => $this->profileFor($request->user('whisperly')),
        ]);
    }

    public function admin(): View
    {
        $talents = pengguna::query()
            ->where('role', 'talent')
            ->orderBy('username')
            ->get()
            ->map(fn (pengguna $user) => $this->profileFor($user));

        return view('admin', compact('talents'));
    }

    public function edit(Request $request): View
    {
        $user = $request->user('whisperly');

        return view('whisperly.talents.edit', [
            'talent' => $this->profileFor($user),
        ]);
    }

    public function update(Request $request): RedirectResponse
    {
        $request->validate([
            'description' => ['required', 'string', 'max:2000'],
            'photo' => ['nullable', 'image', 'max:2048'],
            'schedule' => ['required', 'array'],
            'schedule.*' => ['required', 'in:available,unavailable,booked'],
        ]);

        $user = $request->user('whisperly');
        $talent = $this->profileFor($user);
        $talent->deskripsi = $request->string('description')->toString();

        if ($request->hasFile('photo')) {
            if ($talent->photo) {
                Storage::disk('public')->delete($talent->photo);
            }
            $talent->photo = $request->file('photo')->store('talent-profiles', 'public');
        }

        $talent->save();

        foreach ($talent->schedules as $schedule) {
            if ($request->has("schedule.{$schedule->id}")) {
                $schedule->update(['status' => $request->input("schedule.{$schedule->id}")]);
            }
        }

        return redirect()->route('talent')->with('status', 'Profil dan jadwal berhasil diperbarui.');
    }

    private function profileFor(pengguna $user): talents
    {
        $profile = talents::firstOrCreate(
            ['pengguna_id' => $user->id],
            ['deskripsi' => 'Belum ada deskripsi talent.']
        );

        foreach (self::SLOTS as [$start, $end]) {
            TalentSchedule::firstOrCreate(
                ['talent_id' => $profile->id, 'start_time' => $start, 'end_time' => $end],
                ['status' => 'unavailable']
            );
        }

        return $profile->load(['pengguna', 'schedules', 'ratings']);
    }
}

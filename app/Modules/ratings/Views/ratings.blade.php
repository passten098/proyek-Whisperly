            ...
            <div class="card-body">
                <div class="row align-items-center mb-3 g-2">
                    <div class="col-12 col-md-9">
                        <form action="{{ route('ratings.index') }}" method="get">
                    ...
                    <table class="table table-hover align-middle kt-table" id="table1">
                        ...
                        <tbody>
                            ...
                            @forelse ($data as $item)
                                <tr>
                                    <td>{{ $no++ }}</td>
                                    <td>{{ $item->booking?->talent?->nama_talent ?? '-' }}</td>
									<td>{{ $item->pengguna?->username ?? '-' }}</td>
									<td>{{ $item->nilai_rating }}</td>
									<td>{{ $item->ulasan }}</td>
									
                                    <td>
										{!! button('ratings.show','', $item->id) !!}
										{!! button('ratings.edit', $title, $item->id) !!}
                                        {!! button('ratings.destroy', $title, $item->id) !!}
                                    </td>
                                </tr>
                            @empty
                                <tr>

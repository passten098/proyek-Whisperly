...
{ 
    ...
    public function riwayat(Request $request, talents $talents)
    {
        $data['talent'] = $talents->load([
            'pengguna',
            'bookings',
            'ratings',
        ]);

        $text = 'melihat riwayat '.$this->title;
        $this->log($request, $text, ['talents.id' => $talents->id]);

        return view('talents::talents_riwayat', array_merge($data, [

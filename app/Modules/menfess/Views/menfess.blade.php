            ...
            <div class="card-body">
                <div class="row align-items-center mb-3 g-2">
                    <div class="col-12 col-md-9">
                        <form action="{{ route('menfess.index') }}" method="get">
                    ...
                    <table class="table table-hover align-middle kt-table" id="table1">
                        ...
                        <tbody>
                            ...
                            @forelse ($data as $item)
                                <tr>
                                    <td>{{ $no++ }}</td>
                                    <td>{{ $item->pengguna?->username ?? 'Anonim' }}</td>
                                    <td>{{ $item->kategori?->jenis_kategori ?? '-' }}</td>
									<td>{{ $item->isi_pesan }}</td>
									<td>{{ $item->status }}</td>
									
                                    <td>
										{!! button('menfess.show','', $item->id) !!}
										{!! button('menfess.edit', $title, $item->id) !!}
                                        {!! button('menfess.destroy', $title, $item->id) !!}
                                    </td>
                                </tr>
                            @empty
                                <tr>
